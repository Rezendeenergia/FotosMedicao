...
def resize_photo(img_bytes: bytes) -> bytes:
    """Redimensiona imagem para o tamanho exato com crop central"""
    try:
...
def duplicate_slide(prs: Presentation, source_index: int):
    ...
    # Copiar relacionamentos de imagem
...
def process_pptx(pptx_bytes: bytes, photos: list) -> bytes:
    ...
    for slide_idx, slide in enumerate(all_slides[1:], start=1):
        ...
        # Log de progresso a cada 5 slides
        if slide_idx % 5 == 0 or photo_idx >= n_photos:
            elapsed = time.time() - start_slide_time
            logger.info(f"Progresso: {photo_idx}/{n_photos} fotos inseridas em {slides_used} slides ({elapsed:.1f}s)")
    ...
    total_time = time.time() - start_slide_time
...
def process():
    """Processa o relatório com template e fotos"""
...
def validate():
    """Valida os arquivos antes do processamento"""
    result = {}
